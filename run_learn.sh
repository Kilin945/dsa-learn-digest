...
MODE="${1:-daily}"
SUBJECT_DAILY="每日 DSA"
SUBJECT_WEEKLY="每週 DSA 回顧"
...
run_claude() {  # $1=prompt 字串 $2=輸出檔
  ...
  CLAUDE_LAST_ERR="${TMPDIR:-/tmp}/dsa-learn-claude-err"
  : > "$CLAUDE_LAST_ERR"
...
do_prepare_batch() {
  ...
  tail_dir="$(mktemp -d -t dsa-learn-batch)"
  base="$(cat "$DIR/prompt_daily.txt")"
  tmp="$(mktemp -t dsa-learn)"
...
do_send() {
  ...
  if print -r -- "$ctx" | grep -q '^STATUS: FINISHED'; then
    if send_html "$DONE_HTML" "DSA 課綱完成"; then
      date '+%Y-%m-%d %H:%M:%S' > "$MARKER_DAILY"
...
do_prepare_weekly() {
  ...
  if [ -n "$as_of" ]; then
    ...
    # 連同預計寄出日寫成一份暫存歷史。
    tail_dir="$(mktemp -d -t dsa-learn-weekly)"
...
# 最後一班仍然失敗 → 當下直接寄警示信。
# 這裡特別重要：dsa-learn 備的是「明天的課」，等雲端通知等於事發 25 小時後才知道
