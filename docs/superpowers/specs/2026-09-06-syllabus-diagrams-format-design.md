...
課綱 44 條裡 35 條有對應章節。以下 **9 條**沒有對應圖：陣列上的掃描技巧、字串 String、鏈結串列的指標技巧、樹的經典題型（LCA/直徑）、拓撲排序、最短路徑入門、答案空間二分、排序的應用、面試解題流程總整理。這些主題 `diagram_map.json` 給空陣列，那幾天的信就是純文字。

缺口集中在兩類：**解題技巧**（hello-algo 是資料結構教科書，不收技巧章）與**總結型主題**（本來就沒有單一結構可畫）。
